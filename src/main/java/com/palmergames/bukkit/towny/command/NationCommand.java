...
import com.palmergames.bukkit.towny.exception.EmptyNationException;
...
import com.palmergames.bukkit.towny.question.towny.JoinNationTask;
import com.palmergames.bukkit.towny.question.towny.ResidentNationQuestionTask;
    ...
    public void nationAssistantsRemove(Player player, Resident resident, Nation nation, List<Resident> kicking) {
        ...
        if (kicking.size() > 0) {
            ...
            for (Resident member : kicking) {
                 ...
                 *
                 * Player p = plugin.getServer().getPlayer(member.getName()); if
                 * (p != null)
                 * p.sendMessage(String.format(TownySettings.getLangString("msg_lowered_to_res_by"),
                 * player.getName()));
                 */
            }
            msg = msg.substring(0, msg.length() - 2);
            ...
            if (split[0].equalsIgnoreCase("neutral")) {
                try {
                    ...
                    if (TownySettings.isUsingEconomy() && cost > 0) {
                        TownyMessaging.sendMsg(player, String.format(TownySettings.getLangString("msg_you_paid"), TownyEconomyObject.getFormattedBalance(cost)));
                    } else {
