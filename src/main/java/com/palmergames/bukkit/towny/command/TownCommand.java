...
import com.palmergames.bukkit.towny.exception.AlreadyRegisteredException;
import com.palmergames.bukkit.towny.exception.EconomyException;
import com.palmergames.bukkit.towny.exception.EmptyTownException;
...
import com.palmergames.bukkit.towny.question.towny.JoinTownTask;
import com.palmergames.bukkit.towny.question.towny.ResidentTownQuestionTask;
...
public class TownCommand implements CommandExecutor {
    ...
    private void parseTownCommand(Player player, String[] split) {
        ...
        if (split.length == 0) {
            ...
            } else // TODO: Check if player is an admin
            {
        ...
        if (split.length == 0) {
            ...
            if (split[0].equalsIgnoreCase("board")) {
                ...
                if (split.length < 2) {
                    ...
                    return;
                } else if (plugin.isPermissions() && (!TownyUniverse.getPermissionSource().hasPermission(player, PermissionNodes.TOWNY_TOWN_RENAME.getNode()))) {
    ...
    public static int townBuyBonusTownBlocks(Town town, int inputN) throws TownyException {
        ...
        try {
            ...
            if (TownySettings.isUsingEconomy() && !town.pay(cost, String.format("Town Buy Bonus (%d)", n))) {
                throw new TownyException(String.format(TownySettings.getLangString("msg_no_funds_to_buy"), n, "bonus town blocks", TownyEconomyObject.getFormattedBalance(cost)));
            }
    ...
    public static void checkIfSelectionIsValid(TownBlockOwner owner, List<WorldCoord> selection, boolean attachedToEdge, double blockCost, boolean force) throws TownyException {
        ...
        try {
            ...
            if (TownySettings.isUsingEconomy() && !owner.canPayFromHoldings(cost)) {
                throw new TownyException(String.format(TownySettings.getLangString("msg_err_cant_afford_blocks"), selection.size(), TownyEconomyObject.getFormattedBalance(cost)));
            }
        } catch (EconomyException e1) {
            throw new TownyException("Economy Error");
        }
     ...
     * TownyException { try { TownBlock townBlock = worldCoord.getTownBlock();
     * try { throw new
     * AlreadyRegisteredException(String.format(TownySettings.getLangString("msg_already_claimed"),
     * townBlock.getTown().getName())); } catch (NotRegisteredException e) {
     * throw new
     * AlreadyRegisteredException(TownySettings.getLangString("msg_already_claimed_2"));
     * } } catch (NotRegisteredException e) { TownBlock townBlock =
     * worldCoord.getWorld().newTownBlock(worldCoord); townBlock.setTown(town);
     * if (!town.hasHomeBlock()) town.setHomeBlock(townBlock); if
     * (town.getWorld().isUsingPlotManagementRevert()) { PlotBlockData plotChunk
     * = TownyRegenAPI.getPlotChunk(townBlock); if (plotChunk != null) {
     * TownyRegenAPI.deletePlotChunk(plotChunk); // just claimed so stop
     * regeneration. } else { plotChunk = new PlotBlockData(townBlock); // Not
     * regenerating so create a new snapshot. plotChunk.initialize(); }
     * TownyRegenAPI.addPlotChunkSnapshot(plotChunk); // Save a snapshot.
     * plotChunk = null; } return true; } }
     *
     * public static boolean townUnclaim(Town town, WorldCoord worldCoord,
     * boolean force) throws TownyException { try { TownBlock townBlock =
     * worldCoord.getTownBlock(); if (town != townBlock.getTown() && !force)
     * throw new
     * TownyException(TownySettings.getLangString("msg_area_not_own"));
     *
     * plugin.getTownyUniverse().removeTownBlock(townBlock);
     *
     * return true; } catch (NotRegisteredException e) { throw new
